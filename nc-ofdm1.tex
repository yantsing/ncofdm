...
Compared with the method in \cite{4411634}, our algorithms do not need a trade-off parameter to control the relative priority of the OBP and the PAPR requirements.   Compared with the method in \cite{ni2015joint}, we reserve some tones to control the OBP and the PAPR.  The method in \cite{ni2015joint} does not need to reserve tones.  However, by extending the constellation points in specific directions, it increases the total transmit power.
...
\end{eqnarray}
Problem \eqref{eq:projector_B} cannot be decomposed into a simpler form, and convex optimization tools such as the CVX \cite{cvx} need to be used to solve it.
