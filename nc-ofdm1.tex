...
%\EICaddress{\texttt{[address]}}
%}
\paffiliate[labelC]{Presently, the author is with the Department of Communication Engineering, Jiangxi University of Finance and Economics.}
%\received{2015}{10}{8}
%\revised{2015}{10}{31}
...
Compared with the method in \cite{4411634}, our algorithms do not need a trade-off parameter to control the relative priority of the OBP and the PAPR requirements.  Compared with the method in \cite{ni2015joint}, we reserve some tones to control the OBP and the PAPR.  The method in \cite{ni2015joint} does not need to reserve tones.  However, by extending the constellation points in specific directions, it increases the total transmit power.  With preliminary results reported in our conference paper \cite{6831614}, this paper has substantially extended work in the following aspects. First, the primary user subcarriers are selected to carry optimizing weights, besides the reserved tones, to improve the performance. Second, we study a practical NC-OFDM system with the cyclic prefix and the interpolation filter. The PAPR is calculated through oversampling. Third, the performances of the algorithms are compared with each other and with other methods such as the  constellation extension method.  Fourth, we propose an algorithm which reduces the complexity of the projection onto the set constrained by the OBP limit.
...
\begin{figure}[t]
	%Got by program pocs1.m, pocs2.m, pocs3.m%
	\centering
	\includegraphics[scale = 0.5]{spectrums.pdf}
...
%The CE method can improve the OBP reduction in the two bands by about 3 dB. The performance of the OBP reduction of Algorithm \ref{alg:POCS2} is in between the performance of the other two methods.
...
%\appendix
%\appendix*
\profile[IMG_6375.pdf]{Yanqing Liu}{received the B.S. degree in applied electronics and the
...
, where he participated in the design of TD-SCDMA
and TD-LTE system. Presently, he is with the Department of Communication Engineering, Jiangxi University of Finance and Economics. His research interests include cognitive
radio network, communication systems, wireless networks, digital signal
processing, and cyber-physical systems.}

\profile[227755.pdf]{Liang Dong}{received the B.S. degree in applied physics with minor
...
Engineering. His research interests include communication systems, wireless networks, digital
